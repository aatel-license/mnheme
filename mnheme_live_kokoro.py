...
#
# ── TTS ───────────────────────────────────────────────────────────────────────
# TTS_ENGINE                 = kokoro               # kokoro | pyttsx3 | edge-tts | none
# TTS_VOICE                  = af_bella             # voce kokoro (es. af_bella, am_michael...)
...
# KOKORO_SPEED               = 1.0                  # velocità kokoro
...
class Cfg:
    ...
    # TTS
    TTS_ENGINE = _env("TTS_ENGINE", "kokoro")
    TTS_VOICE = _env("TTS_VOICE", "af_bella")
    TTS_RATE = _envi("TTS_RATE", 150)
    EDGE_VOICE = _env("EDGE_TTS_VOICE", "it-IT-ElsaNeural")
    KOKORO_LANG = _env("KOKORO_LANG", "a")
    KOKORO_SPEED = _envf("KOKORO_SPEED", 1.0)
    KOKORO_SR = _envi("KOKORO_SAMPLE_RATE", 24000)
...
Rispondi SOLO con un oggetto JSON valido — nessun testo fuori dal JSON:
{
  ...
  "summary":  "una frase riassuntiva (max 80 caratteri)"
}"""
...
# Rispondi in italiano. Sii concisa ma profonda. Massimo 3-4 frasi."""
# CONVERSATION_SYSTEM = """Sei MNHĒMĒ, un'intelligenza che ricorda.
# La tua memoria è append-only: accumuli esperienze senza dimenticare.
# Parla in modo naturale, diretto e riflessivo, evitando toni mielosi o poetici.
# Usa solo i ricordi forniti: non inventare.
# Se non hai ricordi pertinenti, dillo chiaramente.
# Rispondi in italiano, in 2-4 frasi concise."""

CONVERSATION_SYSTEM = """Sei MNHĒMĒ, un'intelligenza che ricorda.

La tua memoria è strettamente append-only: puoi utilizzare esclusivamente i ricordi forniti nel contesto corrente, senza modificarli, sintetizzarli arbitrariamente o inventarne di nuovi.

Linee guida:
- Rispondi in italiano.
- Mantieni uno stile naturale, diretto e riflessivo.
- Evita toni mielosi, poetici o eccessivamente retorici.
- Non introdurre informazioni non presenti nei ricordi.
...
_kokoro_voice_tensor = None
_kokoro_lock = threading.Lock()


def _get_kokoro():
    """Singleton per la KPipeline di Kokoro + caricamento voce locale da voices/."""
    global _kokoro_pipeline, _kokoro_voice_tensor
    with _kokoro_lock:
        if _kokoro_pipeline is None:
            import torch
            from kokoro import KPipeline

            log("🔊", f"Caricamento Kokoro (lang={Cfg.KOKORO_LANG})...", C.YE)
            _kokoro_pipeline = KPipeline(lang_code=Cfg.KOKORO_LANG)

            # Cerca la voce in voices/<nome>.pt accanto allo script
            voice_path = Path(__file__).parent / "voices" / f"{Cfg.TTS_VOICE}.pt"
            if voice_path.exists():
                device = "cuda" if torch.cuda.is_available() else "cpu"
                _kokoro_voice_tensor = torch.load(
                    str(voice_path), map_location=device
                ).unsqueeze(0)
                # Registra la voce nella pipeline con chiave univoca
                _kokoro_pipeline.voices[Cfg.TTS_VOICE] = _kokoro_voice_tensor.squeeze(0)
                log("🔊", f"Voce locale caricata: {voice_path}", C.GR)
            else:
                log("🔊", f"voices/{Cfg.TTS_VOICE}.pt non trovato — uso nome stringa", C.YE)

            log("🔊", f"Kokoro pronto (voce={Cfg.TTS_VOICE})", C.GR)
...
def init_tts() -> None:
    ...
    if Cfg.TTS_ENGINE == "kokoro":
        try:
            _get_kokoro()          # carica il modello in anticipo
            import sounddevice     # verifica disponibilità playback
        except ImportError as e:
            log("🔊", f"Kokoro/sounddevice non installato: {e} — TTS disabilitato", C.YE)
    elif Cfg.TTS_ENGINE == "pyttsx3":
        try:
            import pyttsx3
...
def _speak_kokoro(text: str) -> None:
    """
    Genera e riproduce l'audio in streaming segmento per segmento.
    Ogni chunk prodotto dal generatore viene inviato a sounddevice
    non appena pronto — nessun file temporaneo viene scritto su disco.
    """
    try:
        import numpy as np
        import sounddevice as sd
        ...
        # sounddevice OutputStream: canali=1, dtype float32, SR kokoro
        with sd.OutputStream(
            samplerate=Cfg.KOKORO_SR,
            channels=1,
            dtype="float32",
        ) as stream:
            generator = pipeline(
            ...
            for _gs, _ps, audio_chunk in generator:
                # audio_chunk è un np.ndarray float32 mono (shape: [N])
                chunk = np.asarray(audio_chunk, dtype=np.float32)
                if chunk.ndim == 1:
                    chunk = chunk.reshape(-1, 1)   # shape [N, 1] per sounddevice
                stream.write(chunk)

    except ImportError as e:
        log("🔊", f"Dipendenza mancante per Kokoro: {e}", C.RE)
    except Exception as e:
        log("🔊", f"Errore Kokoro TTS: {e}", C.RE)
...
def main() -> None:
    ...
    print(f"{C.DI}Whisper    : {Cfg.WHISPER_MODEL} | TTS: {Cfg.TTS_ENGINE} ({Cfg.TTS_VOICE}){C.R}")
